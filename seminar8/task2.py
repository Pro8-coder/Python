...
равняется 15, количество ячеек в ряду — 5. Тогда метод make_order() вернет
строку: **\n\n*.
...
class Cell:
    ...
    def __sub__(self, other):
        if self.quantity - other.quantity > 0:
            ...
            return ValueError('Итоговое значение <= 0')
    ...
    def make_order(self, cells_row):
        index = self.quantity + self.quantity // cells_row + 1
        return ''.join(
            '\n' if not i % (cells_row + 1) else '*' for i in range(1, index))
