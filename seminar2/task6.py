...
def add_product():
    global id_product
    p = (id_product,
         {'название': input('название: '), 'цена': float(input('цена: ')),
          'количество': int(input('количество: ')), 'ед': input('ед: ')})
    id_product += 1
    return products.append(p)


while True:
    add_product()
    if input('Для выхода введите "q": ') == 'q':
        break

dt = dict().fromkeys(['название', 'цена', 'количество', 'ед'])
for i in dt:
    f = []
    for j in products:
        f.append(j[1][i])
